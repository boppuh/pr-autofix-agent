...
from pr_agent.models import BugbotComment, ClassificationLabel, ReviewThread
...
def test_anthropic_classify_parses_json(monkeypatch):
    ...
    assert cls.label is ClassificationLabel.AUTO_FIXABLE
...
def test_openai_classify_parses_json(monkeypatch):
    ...
    fake_client.responses.create.return_value = _openai_response(
        '{"label": "human_required", "confidence": 0.95, "reason": "arch"}'
    )
